        ...
        <div className="fixed inset-0 bg-black/60 z-[999] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="relative h-32 bg-orange-500 p-6 flex items-end">
              <button onClick={() => setShowModal(false)} className="absolute top-4 right-4 bg-white/20 text-white p-2 rounded-full"><X size={20} /></button>
            ...
            <div className="p-6">
                  ...
                  return (
                    <button 
                      ...
                      className={`px-4 py-2 rounded-xl border-2 font-bold transition-all flex items-center gap-2
                        ${selectedVar === name ? 'border-orange-500 bg-orange-50 text-orange-600' : 'border-gray-100 text-gray-500'}`}
                    >
              ...
              <textarea 
                ...
                className="w-full p-4 mt-2 border-2 border-gray-100 rounded-2xl bg-gray-50 outline-none focus:border-orange-500 h-24 resize-none"
                value={note}
                onChange={(e) => setNote(e.target.value)}
              />

              <div className="flex gap-3 mt-8">
                <button onClick={() => setShowModal(false)} className="flex-1 py-4 font-bold text-gray-400">Batal</button>
                <button onClick={confirmAdd} className="flex-[2] py-4 bg-orange-500 text-white rounded-2xl font-bold shadow-lg">Tambahkan</button>
              </div>
            </div>
          </div>
        </div>
