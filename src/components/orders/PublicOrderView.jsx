...
const PublicOrderView = ({ onAddToCart, orderDateInfo, onChangeDate }) => {
  ...
  // Loading state yang lebih rapi
  if (loading) {
    return (
      <div className="bg-gray-50 min-h-screen p-4 space-y-4">
        <div className="h-32 bg-white rounded-3xl animate-pulse w-full" />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="aspect-square bg-white rounded-[2rem] animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      {/* FIXED STICKY HEADER - Pastikan parent tidak memiliki overflow-hidden */}
      <div className="sticky top-0 z-[100] bg-white shadow-md shrink-0 border-b border-gray-100">
        {/* Kasir Info Bar */}
      ...
      {/* GRID KONTEN - Alami (Natural Scroll) */}
