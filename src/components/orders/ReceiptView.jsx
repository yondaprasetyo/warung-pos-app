...
const ReceiptView = ({ order, onBack }) => {
  ...
  // 3. Fungsi Batal Pesanan oleh Pengguna
  ...
  const getStatusUI = (status) => {
    ...
    if (s === 'cancelled' || s === 'batal') return { style: 'bg-red-50 border-red-200 text-red-700', icon: <XCircle size={48} className="text-red-500" />, title: 'DIBATALKAN', desc: 'Maaf, pesananmu dibatalkan.' };
  ...
  return (
    <div className="max-w-md mx-auto p-4 mb-24 animate-in fade-in duration-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in duration-200">
            <div className="bg-orange-500 p-5 flex justify-between items-center text-white">
              <h3 className="font-black italic uppercase flex items-center gap-2 tracking-tighter"><QrCode size={20} /> Bayar QRIS</h3>
            ...
            <div className="p-8 flex flex-col items-center gap-6">
              ...
              <div className="text-center">
                <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em]">Total Pembayaran</p>
              ...
              <button onClick={handleConfirmViaWA} className="w-full bg-[#25D366] text-white py-5 rounded-2xl font-black flex justify-center items-center gap-3 shadow-lg shadow-green-200 active:scale-95 transition-all uppercase italic tracking-wider">
                <MessageCircle size={22} /> Konfirmasi Pembayaran
              </button>
            </div>
          </div>
        </div>
      ...
      {/* 3. KONTEN STRUK + TOMBOL DOWNLOAD KECIL */}
      <div className="relative mb-6 mx-auto" style={{ width: '380px', maxWidth: '100%' }}>
        
        {/* Tombol Simpan Struk (Dipindah ke atas kanan struk) */}
        <button 
          onClick={handleSaveImage} 
          disabled={isSaving} 
          className="absolute top-4 right-4 z-10 bg-gray-100 hover:bg-gray-200 border border-gray-200 text-gray-700 px-3 py-2 rounded-xl flex items-center justify-center gap-2 active:scale-95 shadow-sm transition-all disabled:opacity-70"
          title="Simpan Struk"
        >
          {isSaving ? <Loader2 size={14} className="animate-spin text-orange-500" /> : <Download size={14} />}
        ...
        <div 
          id="receipt-capture" 
          ref={receiptRef} 
          className="bg-white p-6 leading-tight text-gray-800 shadow-sm border border-gray-100 rounded-xl" 
          style={{ 
            width: '100%',
        ...
        >
          {/* Header Struk */}
          <div className="text-center mb-4 border-b-2 border-black pb-4 pt-2">
      ...
      {/* 4. TOMBOL EDIT / BATAL PESANAN (Hanya muncul jika Pending) */}
      {isPending && (
        <div className="grid grid-cols-2 gap-3 mb-6 mx-auto" style={{ width: '380px', maxWidth: '100%' }}>
          <button onClick={handleUserEdit} className="py-3.5 bg-white border-2 border-orange-500 text-orange-600 font-black rounded-2xl flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-sm hover:bg-orange-50">
            <Edit2 size={16} /> Edit Pesanan
          </button>
          <button onClick={handleUserCancel} className="py-3.5 bg-white border-2 border-red-500 text-red-600 font-black rounded-2xl flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-sm hover:bg-red-50">
            <X size={16} /> Batalkan Pesanan
          </button>
        </div>
      )}

      {/* 5. TOMBOL BAYAR CEPAT */}
      {canPayNow && (
        <div className="mx-auto mb-8" style={{ width: '380px', maxWidth: '100%' }}>
          <button 
            onClick={() => setShowPaymentModal(true)} 
            className="w-full bg-gradient-to-r from-orange-500 to-red-600 text-white p-5 rounded-[2.5rem] shadow-xl shadow-orange-100 flex items-center justify-center gap-4 animate-pulse hover:animate-none transition-all active:scale-95"
          >
            <QrCode size={28} />
      ...
      {/* 6. PANEL KONTROL ADMIN */}
      {currentUser && currentUser.role === 'admin' && (
        <div className="mt-8 p-6 bg-gray-900 rounded-[2.5rem] border-2 border-orange-500 shadow-xl shadow-orange-100/20 animate-in slide-in-from-bottom duration-500 mx-auto" style={{ width: '380px', maxWidth: '100%' }}>
      ...
      {/* 7. ACTION FOOTER (Bantuan WA) */}
      <div className="mt-8 flex flex-col gap-3 mx-auto" style={{ width: '380px', maxWidth: '100%' }}>
